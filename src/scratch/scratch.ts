import { IScratchpadItem, ScratchpadItemType } from "./interfaces";
...
export const saveScratches = (items: IScratchpadItem[]) => writeToLocalStorage<IScratchpadItem[]>('WTScratchpadItems', items);
...
export const removeScratch = (scratches: IScratchpadItem[], id: string): IScratchpadItem[] => (scratches.filter(item => item.id !== id) ?? []);

export const updateScratch = (scratches: IScratchpadItem[], sratch: IScratchpadItem) => {
	let items = scratches.filter(item => item.id !== sratch.id) ?? [];
	items.push(sratch);
	return items;
};
